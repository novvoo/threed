...
- **模型列表**：动态列出 `public/models/` 下所有模型（按修改时间倒序，最新的在最前并默认加载），同名的多格式产物（.ply + .splat）聚合为一个条目，点击优先加载 .ply；往目录里放新模型文件或完成一次转换，列表都会自动更新
...
- **模型下载**：当前展示的模型一键打包下载为 zip（该模型的全部格式文件 .ply / .splat + 内嵌模型数据的独立预览 HTML，双击 HTML 即可在浏览器查看，需联网加载 three.js CDN）；本地拖入的文件不提供下载
